...
- `Out of scope`: not required for the V2 migration completion criteria
...
| hooks | `test/hooks/acrossBridgeHook.spec.ts` | `test-foundry/integration/AcrossBridgeHook.t.sol` | Out of scope | Legacy V1 bridge hook; existing Foundry port may remain, but this row no longer gates V2 migration completion |
...
| periphery | `test/periphery/protocolViewer.spec.ts` | `test-foundry/integration/ProtocolViewer.t.sol` | Out of scope | Legacy V1 viewer; existing Foundry port may remain, but only `ProtocolViewerV2` is required for completion |
...
| escrowV2 | `test/escrowV2/escrowV2.legacyCoverage.spec.ts` | `test-foundry/integration/EscrowV2LegacyCoverage.t.sol` | Mapped | Planned split: deposit management, intent lifecycle, and residual oracle/rate-manager plus view coverage |
| escrowV2 | `test/escrowV2/escrowV2.oracleRates.spec.ts` | `test-foundry/integration/EscrowV2OracleRates.t.sol` | Ported | 29 Foundry tests preserve inline oracle config wiring, single and batch setters, spread math edge cases, oracle-halt branches, currency-update removal semantics, deactivation cleanup, and unsupported tuple guards |
| escrowV2 | `test/escrowV2/escrowV2.pythOracle.spec.ts` | `test-foundry/integration/EscrowV2PythOracle.t.sol` | Ported | 5 Foundry tests preserve Pyth oracle config application, fixed-vs-oracle floor selection, stale-price halts, live price updates, and inline createDeposit oracle config wiring |
| orchestratorV2 | `test/orchestratorV2/orchestratorV2.legacyCoverage.spec.ts` | `test-foundry/integration/OrchestratorV2Legacy*.t.sol` | Ported | 48 residual Foundry tests across `OrchestratorV2LegacyHooksAndRelease` and `OrchestratorV2LegacyFulfillAndSignal`, plus existing `PreIntentHook` and `WhitelistPreIntentHook` coverage, preserve cancel/release/pruning/governance flows, fulfill and post-hook branches, referral-fee validation, and gating-signature behavior |
...
| deploy | `test/deploy/00_system.spec.ts` | `test-foundry/integration/deploy/SystemV1Deployment.t.sol` | Out of scope | Legacy V1 system deploy; existing Foundry port may remain, but completion is gated only by deploy `14-17` |
| deploy | `test/deploy/01_unifiedVerifier.spec.ts` | `test-foundry/integration/deploy/UnifiedVerifierDeployment.t.sol` | Out of scope | Legacy V1/shared deploy flow; existing Foundry port may remain, but this row no longer gates completion |
| deploy | `test/deploy/02_venmoPaymentMethod.spec.ts` | `test-foundry/integration/deploy/VenmoPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/03_revolutPaymentMethod.spec.ts` | `test-foundry/integration/deploy/RevolutPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/04_cashappPaymentMethod.spec.ts` | `test-foundry/integration/deploy/CashappPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/05_wisePaymentMethod.spec.ts` | `test-foundry/integration/deploy/WisePaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/06_mercadopagoPaymentMethod.spec.ts` | `test-foundry/integration/deploy/MercadopagoPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/07_zellePaymentMethods.spec.ts` | `test-foundry/integration/deploy/ZellePaymentMethodsDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/08_paypalPaymentMethod.spec.ts` | `test-foundry/integration/deploy/PaypalPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/09_monzoPaymentMethod.spec.ts` | `test-foundry/integration/deploy/MonzoPaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
| deploy | `test/deploy/10_acrossBridgeHook.spec.ts` | `test-foundry/integration/deploy/AcrossBridgeHookDeployment.t.sol` | Out of scope | Legacy V1 periphery deploy; existing Foundry port may remain, but V2 deployment completion is gated by `14-17` only |
| deploy | `test/deploy/10_n26PaymentMethod.spec.ts` | `test-foundry/integration/deploy/N26PaymentMethodDeployment.t.sol` | Out of scope | Legacy V1 payment-method deployment; superseded by consolidated V2 payment-method deployment scope in `16_v2PaymentMethods` |
