...
4. Every mutating runner command (`setup`, `fast`, `after-cliff`, or `execute`) must first re-run the chain/config/current-Envio-head safety gate. Direct phase commands never bypass this gate. Fully populate and locally sign each transaction, then persist its exact hash, nonce, chain, gas, fees, and signed payload in the private `0600` run-state before broadcasting the raw transaction. Repo evidence recursively strips signed payloads and long raw hex, and records only allowlisted/sanitized broadcast error metadata. Resume resolves only that precomputed hash and retries incomplete indexer reconciliation; ambiguous/dropped/replaced hashes require explicit operator resolution and never cause a new transaction to be constructed or automatically broadcast.
