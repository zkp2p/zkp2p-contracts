...
| Field | Value |
| --- | --- |
| Status | Plan complete; execution blocked on deployed addresses and synced indexer endpoint |
...
| --- | --- | --- |
| Chain / chain ID | Base staging / verified from RPC | `TBD` |
| RPC hostname | Redacted to hostname; sourced from `.env` | `TBD` |
| Deployer/governance | Approved address derived from `.env`, ending in/known as `0x84…` | `TBD` |
| Contracts git/deployment commit | Merged source plus staging deployment commit | `TBD` |
| Deployment transaction block range | Supplied by deployment task | `TBD` |
| Stake vault | Supplied by deployment task | `TBD` |
| Risk manager | Supplied by deployment task | `TBD` |
| Orchestrator V3 | Supplied by deployment task | `TBD` |
| Escrow V2 instance(s) | Supplied by deployment task | `TBD` |
| Deferred payout hook | Supplied by deployment task | `TBD` |
| USDC/token | Supplied by deployment task; verify decimals on-chain | `TBD` |
...
| Block explorer | Base staging explorer | `TBD` |
...
| --- | --- | --- | --- |
| `GOV` | `0x84…` deployer/governance; configuration and authorized test funding | Existing `.env`; native gas | Yes |
| `OWNER_A` | Primary stake owner; Safe if one is available, otherwise fresh EOA for baseline | USDC stake + gas | Yes |
| `OWNER_B` | Isolation/control owner | Small USDC stake + gas | Yes |
| `TAKER_A1` | Relayer/taker authorized by `OWNER_A` | Gas only | Yes |
| `TAKER_A2` | Second relayer using the same `OWNER_A` portfolio | Gas only | Yes |
| `TAKER_B` | Taker backed by `OWNER_B` | Gas only | Yes |
| `LP_A` | Creates controlled deposits and receives penalties/chargeback compensation | Deposit USDC + gas | Yes |
| `LP_B` | Isolation and LP-grouped exposure checks | Deposit USDC + gas | Yes |
| `RECIPIENT` | Intent proceeds recipient | Gas optional | Yes |
| `UNAUTHORIZED` | Negative calls and attempted stake use | Gas only | Yes |
| `CALLER` | Permissionless maturity/reconciliation/batch caller | Gas only | Yes |
...
This section is intentionally empty until deployment handoff. Add one row per granular case; do not collapse multiple cases into one result.

| Case | Result | UTC | Tx/block | Contract evidence | Indexer evidence | Notes/bug |
| --- | --- | --- | --- | --- | --- | --- |
| `TBD` | `NOT RUN` | — | — | — | — | Awaiting deployment handoff. |
