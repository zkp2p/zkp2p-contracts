...
4. Every mutating runner command (`setup`, `fast`, `after-cliff`, or `execute`) must first re-run the chain/config/current-Envio-head safety gate. Direct phase commands never bypass this gate. Immediately journal each returned transaction hash before waiting for its receipt; resume resolves that exact hash and retries incomplete indexer reconciliation, while dropped/replaced hashes require explicit operator resolution and are never automatically rebroadcast.
