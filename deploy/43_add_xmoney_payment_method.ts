...
export const XMONEY_DEPLOY_TAG = "43_add_xmoney_payment_method";
...
async function assertPaymentBindingState({
  ...
  paymentVerifierRegistry,
  unifiedPaymentVerifierV3,
}: StagingContracts): Promise<{ registryHasMethod: boolean; verifierHasMethod: boolean }> {
  ...
  if (registryHasMethod && !verifierHasMethod) {
    throw new Error("X Money registry binding has no verifier method");
  }
  ...
  const expectedVerifierMethods = (verifierHasMethod
    ? [...predecessorNames, "xmoney"]
    : predecessorNames).map(calculatePaymentMethodHash);
  if (!sameValueSet(verifierMethods, expectedVerifierMethods)) {
    throw new Error(
...
async function simulateMissingWrites({
  paymentVerifierRegistry,
  unifiedPaymentVerifierV3,
}: StagingContracts, state: { registryHasMethod: boolean; verifierHasMethod: boolean }): Promise<void> {
  const paymentMethodHash = XMONEY_PROVIDER_CONFIG.paymentMethodHash;
  if (!state.verifierHasMethod) {
...
const func: DeployFunction = async function (
  ...
  if (await func.skip!(hre)) return;
  const contracts = await loadStagingContracts(hre);
  const state = await assertPaymentBindingState(contracts);
  await simulateMissingWrites(contracts, state);
  ...
  if (!(await assertPaymentBindingState(contracts)).registryHasMethod) {
...
func.skip = async (hre: HardhatRuntimeEnvironment): Promise<boolean> => {
  if (process.env.DEPLOY_ACTIVE_TAG !== XMONEY_DEPLOY_TAG) return true;
