...
| StakeVault deposit, delegation, reservation, slashing, and compensation slice | 30 of 56 | 30 across 2 domain contracts and 1 abstract fixture | Every translated test passed individually; complete files 30/30, 0 skipped; full same-commit Hardhat source 56/56 in real 2.72s; cumulative deterministic suite 1,085/1,085 with zero skips | One-to-one verified exact deposits/liabilities/events, deposit pause and zero gates, delegated ownership/revocation/anti-squatting/preapproval/batch atomicity, reservation uniqueness/free-stake accounting/update/increase/release, pause/exit admission gates including zero-increment refreshes, controller authorization, partial slashing, and aggregated maker compensation. Remaining 26 source rows deliberately remain unmapped. |

Current deterministic mapping: 1,194 / 1,517 rows (78.71%); 323 remain. The complete rebased deterministic suite is 1,085/1,085 with zero skips; all 276 legacy Escrow tests, all 141 legacy Orchestrator tests, the current 30 StakeVault tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed Escrow and Orchestrator sources and the current StakeVault slice are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 pre-intent hook administration/execution/signature gating/reentry containment, dedicated whitelist-hook access control plus dual-hook atomicity, both Across bridge generations' success/fallback/rescue behavior, RateManagerV1 configuration/rates/fees/liquidity/registry control, EscrowV2 delegation plus oracle-configuration integration, OrchestratorV2 lifecycle/pruning/governance/referral/post-hook/gating edge paths, complete Escrow/Orchestrator parity, and StakeVault deposit/delegation/reservation/slashing behavior. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
