...
| RiskManager real-system configuration, free expiry, and admission | 21 of 70 | 21 across 2 domain contracts and 1 abstract full-system fixture | Every translated test passed individually; complete files 21/21, 0 skipped; same-commit Hardhat groups 21/21 (configuration real 0.86s; admission real 1.04s); cumulative deterministic suite 1,150/1,150 with zero skips | One-to-one verified custody binding, config/formula boundaries, non-chargebackable admission/cancellation/pruning without stake charges, amount snapshots, concurrent delegated takers, removal of legacy relayer/global-multiple-intent selectors, delegated Safe ownership, portfolio capacity, insufficient-free-stake rejection, and immutable policy snapshots. The fixture recreates the Hardhat EscrowV2/OrchestratorV3/linked settlement libraries/registries/verifier/StakeVault/RiskManager topology. Remaining 49 source rows deliberately remain unmapped. |

Current deterministic mapping: 1,259 / 1,517 rows (82.99%); 258 remain. The complete rebased deterministic suite is 1,150/1,150 with zero skips; all 276 legacy Escrow tests, all 141 legacy Orchestrator tests, all 56 StakeVault tests, all 18 RiskManager branch tests, the current 21 real-system RiskManager tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed sources and current slices are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 pre-intent hook administration/execution/signature gating/reentry containment, dedicated whitelist-hook access control plus dual-hook atomicity, both Across bridge generations' success/fallback/rescue behavior, RateManagerV1 configuration/rates/fees/liquidity/registry control, complete Escrow/Orchestrator/StakeVault parity, complete RiskManager hard-cut branch parity, and actual OrchestratorV3-linked RiskManager configuration/formula/admission parity. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
