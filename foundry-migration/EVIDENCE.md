...
| RiskManager real-system configuration, admission, extensions, settlement, and chargebacks | 48 of 70 | 48 across 6 domain contracts and 1 abstract full-system fixture | Every translated test passed individually; complete files 48/48, 0 skipped; same-commit Hardhat groups 48/48 (configuration real 0.86s; admission real 1.04s; extensions real 1.31s; settlement/chargebacks real 1.63s); cumulative deterministic suite 1,177/1,177 with zero skips | One-to-one verified custody/config/formulas, free-expiry and portfolio admission, delegated ownership/policy snapshots, extension accounting and boundaries, exact stake-backed fulfillment/manual-release settlement, EIP-712 claim hashing, gross compensation, global dispute replay protection, proof-bound versus witness-bound payment identity, maturity release, and the exact half-open coverage deadline. Remaining 22 source rows deliberately remain unmapped. |

Current deterministic mapping: 1,286 / 1,517 rows (84.77%); 231 remain. The complete rebased deterministic suite is 1,177/1,177 with zero skips; all 276 legacy Escrow tests, all 141 legacy Orchestrator tests, all 56 StakeVault tests, all 18 RiskManager branch tests, the current 48 real-system RiskManager tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed sources and current slices are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 hook and gating edges, Across hooks, RateManagerV1, complete Escrow/Orchestrator/StakeVault parity, complete RiskManager hard-cut branch parity, and actual OrchestratorV3-linked RiskManager configuration/admission/extension/chargeback parity. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
