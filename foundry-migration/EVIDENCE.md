...
Current deterministic mapping: 1,023 / 1,517 rows (67.44%); 494 remain. The complete rebased deterministic suite is 914/914 with zero skips and warm runs of 0.23s, 0.21s, and 0.22s (median 0.22s); all 276 legacy Escrow tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed Escrow sources are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 pre-intent hook administration/execution/signature gating/reentry containment, dedicated whitelist-hook access control plus dual-hook atomicity, both Across bridge generations' success/fallback/rescue behavior, RateManagerV1 configuration/rates/fees/liquidity/registry control, EscrowV2 delegation plus oracle-configuration integration, OrchestratorV2 lifecycle/pruning/governance/referral/post-hook/gating edge paths, the complete EscrowV2 legacy and branch-coverage sources, and the complete legacy Escrow source. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
