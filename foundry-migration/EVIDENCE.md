...
| Legacy Orchestrator constructor, signal-intent, cancellation, fulfillment, and manual release | 100 of 141 | 100 across 7 domain contracts and 1 abstract full-topology fixture | Every translated test passed individually; same-commit Hardhat signal suites 32 passing plus 4 pending in real 5.33s, cancellation 7/7 in real 2.39s, the complete 38-case fulfillment oracle 38/38 in real 7.41s, and the manual-release grep 25/25 in real 6.96s including all 19 legacy manual-release cases; all four pending expiry-reclamation cases execute and pass in Foundry; cumulative deterministic suite 1,014/1,014 with zero skips | One-to-one verified signal validation/reclamation, cancellation cleanup, verifier release math, min snapshots, rate stability, close/config deletion, fee conservation, negative verification precedence, exact post-hook routing/approval cleanup, fee-aware hook settlement, partial-pull rejection, unexpected-balance rejection, partial-payment release/refund/fee behavior, fulfillment reentry containment, normal hook execution, pause behavior, depositor-only manual release, manual close cleanup, protocol/referrer fee conservation, and the manual-release rule that deliberately bypasses post-intent hooks. Remaining 41 source rows deliberately remain unmapped. |

Current deterministic mapping: 1,123 / 1,517 rows (74.03%); 394 remain. The complete rebased deterministic suite is 1,014/1,014 with zero skips; all 276 legacy Escrow tests, the 100 translated legacy Orchestrator tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed Escrow and current Orchestrator source slices are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 pre-intent hook administration/execution/signature gating/reentry containment, dedicated whitelist-hook access control plus dual-hook atomicity, both Across bridge generations' success/fallback/rescue behavior, RateManagerV1 configuration/rates/fees/liquidity/registry control, EscrowV2 delegation plus oracle-configuration integration, OrchestratorV2 lifecycle/pruning/governance/referral/post-hook/gating edge paths, the complete EscrowV2 legacy and branch-coverage sources, the complete legacy Escrow source, and legacy Orchestrator signal/cancellation/fulfillment/manual-release behavior including partial payments, hook reentry containment, manual fee conservation, and all four formerly skipped reclamation cases. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
