...
Current deterministic mapping: 236 / 1,514 rows (15.59%); 1,278 remain. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V3 EIP-712 attestation integrity, legacy/predecessor nullifier replay, bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, and verifier rotation. The V3 core file deliberately records the actual-Orchestrator integration shadow gate as pending rather than overclaiming the interface-harness evidence.
