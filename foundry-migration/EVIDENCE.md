...
| Legacy Escrow constructor, creation, funding, withdrawal, deposit configuration, delegation, lifecycle toggles, and pruning slice | 187 of 276 | 187 across 9 domain contracts and 1 abstract full-topology fixture | Every translated test passed individually; same-commit Hardhat constructor/create/add/remove suites 54/54 in real 7.62s, withdrawal suite 28/28 in real 4.86s, rate/range/payment-method/currency suites 63/63 in real 7.94s, and delegate/accepting/pruning/retain suites 41 passing plus 1 pending in real 6.01s; the pending callback assertion is implemented and passing in Foundry; cumulative deterministic suite 825/825 with zero skips, warm real 0.21s, 0.23s, and 0.21s (median 0.21s) | One-to-one verified complete creation storage/view/events and validation, unrestricted third-party funding, depositor removals, one-way accepting-state demotion, outstanding/expired-intent withdrawal behavior through the real Orchestrator, complete nested configuration cleanup, paused rescue, exact error precedence, conversion-rate/range mutation, payment-method and currency lifecycle, owner-only delegate replacement/removal, liquidity-gated accepting-state transitions, retain-on-empty updates, permissionless expiry pruning, and the exact Orchestrator callback payload. The fixture recreates the production Escrow/Orchestrator/registries/verifier/viewer topology and contains no tests. Mechanical reconciliation proved 187 unique source rows map to 187 unique existing Solidity tests. Remaining 89 rows in this source deliberately remain unmapped. |

Current deterministic mapping: 934 / 1,517 rows (61.57%); 583 remain. The complete rebased deterministic suite is 825/825 with zero skips and warm runs of 0.21s, 0.23s, and 0.21s (median 0.21s); all 187 translated legacy Escrow tests, the 108 EscrowV2 branch tests, the earlier 70 EscrowV2 legacy tests, and the earlier 42 constructor/control-affected V2 tests passed as isolated invocations. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination; completed Escrow source slices are physical one-to-one translations. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, verifier rotation, real OrchestratorV2 settlement, delegated effective-rate enforcement, manager-fee bounds/snapshotting, net settlement accounting, the relayer/multiple-intent hard cut, fixed/Pyth/static-oracle effective-rate behavior including source lifecycle/staleness/signed spread rounding, both legacy/V2 ProtocolViewer aggregation, OrchestratorV2 pre-intent hook administration/execution/signature gating/reentry containment, dedicated whitelist-hook access control plus dual-hook atomicity, both Across bridge generations' success/fallback/rescue behavior, RateManagerV1 configuration/rates/fees/liquidity/registry control, EscrowV2 delegation plus oracle-configuration integration, OrchestratorV2 lifecycle/pruning/governance/referral/post-hook/gating edge paths, the complete EscrowV2 legacy and branch-coverage sources, and the legacy Escrow constructor/create/funding/withdraw/configuration/delegation/state/pruning slice. The verifier core files deliberately record their remaining legacy/V3 actual-Orchestrator shadow gates as pending rather than overclaiming interface-harness evidence.
