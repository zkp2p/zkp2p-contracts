...
Current deterministic mapping: 255 / 1,514 rows (16.84%); 1,259 remain. Consolidations combine assertions from the same Hardhat setup/action only where the manifest gives every source behavior an exact Foundry function destination. Completed slices additionally cover immutable verifier registries, attestation-verifier rotation, payment-method membership/enumeration, exact owner/error/event behavior for the base unified verifier, V1/V3 EIP-712 attestation integrity, nullifier replay, V3 bidirectional payment-to-intent bindings, snapshot validation, overpayment capping, and verifier rotation. The verifier core files deliberately record the actual-Orchestrator settlement shadow gates as pending rather than overclaiming interface-harness evidence.
