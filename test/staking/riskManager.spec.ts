...
describe("RiskManager and OrchestratorV3", () => {
  async function deployFixture() {
    ...
    const orchestratorFeeLib = await (await ethers.getContractFactory("OrchestratorV3FeeLib")).deploy();
    const orchestratorRiskLib = await (await ethers.getContractFactory("OrchestratorV3RiskLib")).deploy();
    const orchestratorValidation = await (await ethers.getContractFactory("OrchestratorV3Validation")).deploy();
    ...
    await vault.initializeController(manager.address);
  ...
  describe("post-funds risk settlement boundary", () => {
    ...
    it("applies deferred custody to manual release and skips the ordinary post hook", async () => {
      const { maker, taker, recipient, escrow, orchestrator, token, vault, manager } =
        await loadFixture(deployFixture);
      await enableDeferred(manager);
      const postHook = await (await ethers.getContractFactory("SettlementHookMock"))
