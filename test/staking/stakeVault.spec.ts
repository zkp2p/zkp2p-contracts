...
describe("StakeVault", () => {
  ...
  describe("reservations", () => {
    ...
    it("increases an active reservation through the admission-gated path", async () => {
      const { controller, staker, vault } = await deployFixture();
      const positionId = ethers.utils.id("extension-top-up");
      ...
      await expect(vault.connect(controller).increaseReservation(positionId, usdc(2), 200))
        .to.emit(vault, "StakeReservationUpdated")
        .withArgs(positionId, staker.address, usdc(1), usdc(3), usdc(3), 200);

      expect(await vault.reservedStake(staker.address)).to.eq(usdc(3));
    ...
    it("rejects invalid reservation increases", async () => {
      ...
      await expect(vault.connect(controller).increaseReservation(positionId, usdc(1), 0))
        .to.be.revertedWithCustomError(vault, "ReservationNotFound");

      await vault.connect(staker).depositStake(usdc(10));
      await vault.connect(controller).reserveStake(staker.address, positionId, usdc(9), 0);
      await expect(vault.connect(controller).increaseReservation(positionId, usdc(2), 0))
        .to.be.revertedWithCustomError(vault, "InsufficientFreeStake");
    });

    it("blocks reservation increases while paused or after exit is requested", async () => {
      const { owner, controller, staker, vault } = await deployFixture();
      const positionId = ethers.utils.id("extension-top-up-gates");
      await vault.connect(staker).depositStake(usdc(10));
      await vault.connect(controller).reserveStake(staker.address, positionId, usdc(1), 100);
      await vault.connect(owner).setStakeOperationsPaused(false, true);
  ...
  describe("controller handover", () => {
    ...
    it("lets the previous controller settle only its snapshotted stake reservations", async () => {
      ...
      await expect(vault.connect(controller).releaseReservation(newIntent))
        .to.be.revertedWithCustomError(vault, "UnauthorizedPositionController");
      await expect(vault.connect(nextController).increaseReservation(oldIntent, usdc(1), 0))
        .to.be.revertedWithCustomError(vault, "UnauthorizedPositionController");
      await expect(vault.connect(controller).increaseReservation(oldIntent, usdc(1), 0))
        .to.be.revertedWithCustomError(vault, "UnauthorizedController");
