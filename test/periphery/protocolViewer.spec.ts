...
describe("ProtocolViewer", () => {
  ...
  describe("#setEscrowContract", async () => {
    let subjectEscrowAddress: Address;
    let subjectCaller: Account;

    async function subject(): Promise<any> {
      return protocolViewer.connect(subjectCaller.wallet).setEscrowContract(subjectEscrowAddress);
    }

    beforeEach(async () => {
      subjectEscrowAddress = receiver.address;
      subjectCaller = owner;
    });

    it("should update escrow contract and emit event", async () => {
      await expect(subject())
        .to.emit(protocolViewer, "EscrowContractUpdated")
        .withArgs(escrow.address, subjectEscrowAddress);

      expect(await protocolViewer.escrowContract()).to.eq(subjectEscrowAddress);
    });

    describe("when escrow is zero address", async () => {
      beforeEach(async () => {
        subjectEscrowAddress = ADDRESS_ZERO;
      });
  ...
  describe("#setOrchestrator", async () => {
    let subjectOrchestratorAddress: Address;
    let subjectCaller: Account;

    async function subject(): Promise<any> {
      return protocolViewer.connect(subjectCaller.wallet).setOrchestrator(subjectOrchestratorAddress);
    }

    beforeEach(async () => {
      subjectOrchestratorAddress = receiver.address;
      subjectCaller = owner;
    });

    it("should update orchestrator and emit event", async () => {
      await expect(subject())
        .to.emit(protocolViewer, "OrchestratorUpdated")
        .withArgs(orchestrator.address, subjectOrchestratorAddress);

      expect(await protocolViewer.orchestrator()).to.eq(subjectOrchestratorAddress);
    });

    describe("when orchestrator is zero address", async () => {
      beforeEach(async () => {
        subjectOrchestratorAddress = ADDRESS_ZERO;
      });
